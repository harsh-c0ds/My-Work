...
! INTEGER, PARAMETER :: number_package_sizes=9
  ...
  TYPE(MPI_Status) :: status, status_arr(2)
...
!              halo-communication                 halo-communication
  ...
  DO j = 1, number_package_sizes
     ...
     DO i = 0, number_of_messages
        ...
        CALL MPI_Isend(snd_buf_left,  length, MPI_REAL, left,  17, MPI_COMM_WORLD, rq(1))
        CALL MPI_Isend(snd_buf_right, length, MPI_REAL, right, 23, MPI_COMM_WORLD, rq(2))

        CALL MPI_Recv (rcv_buf_right, length, MPI_REAL, right, 17, MPI_COMM_WORLD, status)
        CALL MPI_Recv (rcv_buf_left,  length, MPI_REAL, left,  23, MPI_COMM_WORLD, status)
