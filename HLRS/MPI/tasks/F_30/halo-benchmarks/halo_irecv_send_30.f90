...
!              halo-communication                 halo-communication
  ...
  DO j = 1, number_package_sizes
     ...
     DO i = 0, number_of_messages
        ...
        CALL MPI_Irecv(rcv_buf_right, length, MPI_REAL, right, 17, MPI_COMM_WORLD, rq(1))
        CALL MPI_Irecv(rcv_buf_left,  length, MPI_REAL, left,  23, MPI_COMM_WORLD, rq(2))

        CALL MPI_Send (snd_buf_left,  length, MPI_REAL, left,  17, MPI_COMM_WORLD)
        CALL MPI_Send (snd_buf_right, length, MPI_REAL, right, 23, MPI_COMM_WORLD)
