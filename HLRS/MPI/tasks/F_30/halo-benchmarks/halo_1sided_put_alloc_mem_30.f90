...
!              halo-communication                 halo-communication
  ...
  CALL MPI_Alloc_mem(buf_size, MPI_INFO_NULL, ptr_rcv_buf_left)
  CALL C_F_POINTER(ptr_rcv_buf_left, rcv_buf_left, (/max_length/))
  CALL MPI_Win_create(rcv_buf_left,  buf_size, disp_unit, MPI_INFO_NULL, MPI_COMM_WORLD, win_rcv_buf_left)
  CALL MPI_Alloc_mem(buf_size, MPI_INFO_NULL, ptr_rcv_buf_right)
  CALL C_F_POINTER(ptr_rcv_buf_right, rcv_buf_right, (/max_length/))
  CALL MPI_Win_create(rcv_buf_right, buf_size, disp_unit, MPI_INFO_NULL, MPI_COMM_WORLD, win_rcv_buf_right)
  target_disp = 0

  IF (my_rank .EQ. 0) THEN
     WRITE (*,*) "message size   transfertime    duplex bandwidth per process and neighbor"
  END IF
...
!       ... or with MPI-3.0 and later:
        ...
        CALL MPI_Win_fence(MPI_MODE_NOSTORE + MPI_MODE_NOPRECEDE, win_rcv_buf_left)
        CALL MPI_Win_fence(MPI_MODE_NOSTORE + MPI_MODE_NOPRECEDE, win_rcv_buf_right)
        ...
        CALL MPI_Win_fence(MPI_MODE_NOSTORE + MPI_MODE_NOPUT + MPI_MODE_NOSUCCEED, win_rcv_buf_left)
        CALL MPI_Win_fence(MPI_MODE_NOSTORE + MPI_MODE_NOPUT + MPI_MODE_NOSUCCEED, win_rcv_buf_right)
...
!       CALL MPI_Get_address(snd_buf_left,  iadummy)
!       CALL MPI_Get_address(snd_buf_right, iadummy)
!       ... or with MPI-3.0 and later:
...
!       ...snd_buf_... is used to store the values that were stored in snd_buf_... in the neighbor process
  ...
  CALL MPI_Free_mem(rcv_buf_left)
  CALL MPI_Free_mem(rcv_buf_right)
