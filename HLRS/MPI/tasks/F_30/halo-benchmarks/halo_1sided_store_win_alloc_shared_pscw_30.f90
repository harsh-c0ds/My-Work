...
!       ...workaround: no assertions for shared memory - MPI-3.0 and MPI-3.1 are not clear:
        CALL MPI_Win_post (grp_left,  0, win_rcv_buf_left)
        CALL MPI_Win_post (grp_right, 0, win_rcv_buf_right)
        CALL MPI_Win_start(grp_right, 0, win_rcv_buf_left)
        CALL MPI_Win_start(grp_left,  0, win_rcv_buf_right)
...
!       ... or with MPI-3.0 and later:
        ...
        CALL MPI_Win_complete(win_rcv_buf_left )
        CALL MPI_Win_complete(win_rcv_buf_right)
        CALL MPI_Win_wait    (win_rcv_buf_left)
        CALL MPI_Win_wait    (win_rcv_buf_right)
