...
!         CALL MPI_Irecv(rcv_buf(1+length), length, MPI_REAL, right, 17, new_comm, rq(1))
!         CALL MPI_Irecv(rcv_buf(1),        length, MPI_REAL, left,  23, new_comm, rq(2))
!         CALL MPI_Send (snd_buf(1),        length, MPI_REAL, left,  17, new_comm)
!         CALL MPI_Send (snd_buf(1+length), length, MPI_REAL, right, 23, new_comm)
...
! !       IF (.NOT.MPI_ASYNC_PROTECTS_NONBLOCKING) CALL MPI_F_SYNC_REG(rcv_buf)
        ...
        CALL MPI_NEIGHBOR_ALLTOALL(snd_buf, length, MPI_REAL, rcv_buf, length, MPI_REAL, new_comm)
