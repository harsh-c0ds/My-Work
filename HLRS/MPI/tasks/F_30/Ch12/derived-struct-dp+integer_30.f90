...
!    ... to check, whether the data transfer is correct, we do not transfer the last index
     CALL MPI_Issend(snd_buf, arr_lng-1, send_recv_type, right, 17, MPI_COMM_WORLD, request)
     CALL MPI_Recv(rcv_buf, arr_lng-1, send_recv_type, left, 17, MPI_COMM_WORLD, status)
  ...
  if (my_rank==0) THEN
    ...
    WRITE(*,"(a,i3)") "D-- real size is:             ", INT(MPI_Aint_diff(second_var_address,first_var_address))
