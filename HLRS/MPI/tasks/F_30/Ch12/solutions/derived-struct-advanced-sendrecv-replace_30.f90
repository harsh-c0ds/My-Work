...
! Create derived datatype. 
  ...
  CALL MPI_Get_address(buf%i, first_var_address)
  CALL MPI_Get_address(buf%r, second_var_address)
  ...
  sum%i = 0
  sum%r = 0
  buf%i = my_rank           ! Step 1
  buf%r = REAL(10*my_rank)  ! Step 1

  DO i = 1, size
     CALL MPI_Sendrecv_replace(buf, 1, send_recv_type, right, 17,  & ! Step 2
                                                       left, 17,   & ! Step 3
                       MPI_COMM_WORLD, status)
     ! buf = buf            ! Step 4 is not needed because of only one buffer
     sum%i = sum%i + buf%i                                           ! Step 5
     sum%r = sum%r + buf%r                                           ! Step 5
  END DO
