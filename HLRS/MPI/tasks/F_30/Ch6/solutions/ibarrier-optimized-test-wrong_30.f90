...
! ...in the role as sending process
  ...
  IF ((dest>=0) .AND. (dest<size)) THEN
    ...
    CALL MPI_Isend(snd_buf_A,1,MPI_INTEGER, dest,222,MPI_COMM_WORLD, snd_rq(number_of_dests)) ! WRONG, must by synchronous Issend
  ...
  IF ((dest>=0) .AND. (dest<size)) THEN
    ...
    CALL MPI_Isend(snd_buf_B,1,MPI_INTEGER, dest,222,MPI_COMM_WORLD, snd_rq(number_of_dests)) ! WRONG, must by synchronous Issend
  ...
  IF ((dest>=0) .AND. (dest<size)) THEN
    ...
    CALL MPI_Isend(snd_buf_C,1,MPI_INTEGER, dest,222,MPI_COMM_WORLD, snd_rq(number_of_dests)) ! WRONG, must by synchronous Issend
  ...
  IF ((dest>=0) .AND. (dest<size)) THEN
    ...
    CALL MPI_Isend(snd_buf_D,1,MPI_INTEGER, dest,222,MPI_COMM_WORLD, snd_rq(number_of_dests)) ! WRONG, must by synchronous Issend
...
!       Implemented in an optimized way, receiving multiple messages if available
    ...
    DO WHILE(rcv_flag) !   it is no problem to receive as many messages as possible    ! **T**
      ...
      IF(round.EQ.10) THEN          ! only for verification, should be removed in real applications
        ...
        CALL MPI_Iprobe(MPI_ANY_SOURCE,222,MPI_COMM_WORLD, rcv_flag, MPI_STATUS_IGNORE) 
        IF(rcv_flag) THEN
...
!      The following lines make only sense as long as not all MPI_ISSENDs are finished.
    IF(.NOT. snd_finished) THEN
      ...
      CALL MPI_Testall(number_of_dests, snd_rq, snd_finished, MPI_STATUSES_IGNORE)
      ! if all MPI_ISSENDs are finished then call MPI_IBARRIER
      IF(snd_finished) THEN   ! ...i.e., the first time, i.e., only once
        CALL MPI_Ibarrier(MPI_COMM_WORLD, ib_rq)
      END IF
    END IF
!   ...loop until MPI_IBARRIER finished (i.e. all processes signaled that all receives are called)
    IF(snd_finished) THEN ! the test whether the MPI_IBARRIER is finished
      ...
      CALL MPI_Test(ib_rq, ib_finished, MPI_STATUS_IGNORE)
    END IF
  END DO
