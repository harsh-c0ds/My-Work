...
! // ADD ON: calculates the minimum and maximum size of size_shm
  mm(1) = -size_shm ; mm(2) = size_shm

  IF( comm_head /= MPI_COMM_NULL ) THEN
    CALL MPI_Reduce( mm, minmax, 2, MPI_INT, MPI_MAX, 0, comm_head) ;
  ENDIF
  IF( rank_world == 0 ) THEN
    WRITE(*,*)' The number of shared memory islands is: ',size_head,' islands'
    IF ( minmax(1) + minmax(2) == 0 ) THEN
      WRITE(*,*)' The size of all shared memory islands is: ',-minmax(1),' processes'    
    ELSE
      WRITE(*,*)' The size of the shared memory islands is between min=',-minmax(1),' and max=', minmax(2),' processes'    
    ENDIF
  ENDIF
