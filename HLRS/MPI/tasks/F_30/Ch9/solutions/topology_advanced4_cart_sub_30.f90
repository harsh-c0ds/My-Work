...
!==============================================================!
  ...
  INTEGER :: my_coords(max_dims), size_of_slice, my_rank_in_slice 
...
! Compute sum.
  ...
  DO i = 1, size_of_slice
     ...
     CALL MPI_Issend(snd_buf, 1, MPI_INTEGER, right, 17, slice_comm, request)

     CALL MPI_Recv  (rcv_buf, 1, MPI_INTEGER, left,  17, slice_comm, status)
...
!    ... should be substituted as soon as possible by:
  ...
  WRITE(*,*) "PE",my_rank, ", my_coords = (",my_coords(1),",", my_coords(2), &
