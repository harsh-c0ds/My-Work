...
! send and receive to/from upper/lower
    if (kdim.gt.1) then ! otherwise in all processes both, lower and upper are MPI_PROC_NULL
      ...
      call MPI_Irecv(phi(is+b1,ke+1-b1),  1,horizontal_border, upper,2,comm, req(2))
      call MPI_Isend(phi(is+b1,ke+1-2*b1),1,horizontal_border, upper,1,comm, req(3))
      ...
      ! call MPI_Sendrecv(phi(is+b1,ke+1-2*b1),1,horizontal_border, upper,1,  &
      ...
      !               &   phi(is+b1,ke+1-b1),  1,horizontal_border, upper,2, comm, MPI_STATUS_IGNORE)
    endif
...
! send and receive to/from left/right
    if (idim.gt.1) then ! otherwise in all processes both, left and right are MPI_PROC_NULL
      ...
      call MPI_Irecv(phi(ie+1-b1,ks+b1),  1,vertical_border, right,4,comm, req(2))
      call MPI_Isend(phi(ie+1-2*b1,ks+b1),1,vertical_border, right,3,comm, req(3))
      call MPI_Isend(phi(is+b1,ks+b1),    1,vertical_border, left, 4,comm, req(4))
      call MPI_Waitall(4, req, statuses)
      ! or alternatively with sendrecv:
      ! call MPI_Sendrecv(phi(ie+1-2*b1,ks+b1),1,vertical_border, right,3,  &
      !               &   phi(is,ks+b1),       1,vertical_border, left, 3, comm, MPI_STATUS_IGNORE)
      ! call MPI_Sendrecv(phi(is+b1,ks+b1),    1,vertical_border, left, 4,  &
      !               &   phi(ie+1-b1,ks+b1),  1,vertical_border, right,4, comm, MPI_STATUS_IGNORE)
    endif
    comm_time = comm_time + MPI_Wtime() 
