...
!**************************************************************!
  ...
  CALL MPI_Comm______(MPI_COMM_WORLD, world_group)

  IF (mycolor == 0) THEN
    ranges(1,1) = 0                     ! first rank of my range
    ranges(2,1) = (world_size-1)/3      ! last  rank of my range
  ELSE
    ranges(1,1) = ______                ! first rank of my range
    ranges(2,1) = ______                ! last  rank of my range
  END IF
  ...
  CALL MPI_Group___________ ( ___________, _, ______, _________)
  CALL MPI_Comm_______(MPI_COMM_WORLD, _________, sub_comm)
