...
// send and receive to/from upper/lower
    if (kdim > 1) { // otherwise in all processes both, lower and upper are MPI_PROC_NULL
      ...
      MPI_Irecv(&phi[idx(is+b1,ke+1-b1)],  1,horizontal_border, upper,2,comm, &req[1]);
      MPI_Isend(&phi[idx(is+b1,ke+1-2*b1)],1,horizontal_border, upper,1,comm, &req[2]);
      ...
      // MPI_Sendrecv(&phi[idx(is+b1,ke+1-2*b1)],1,horizontal_border, upper,1,
      ...
      //              &phi[idx(is+b1,ke+1-b1)],  1,horizontal_border, upper,2, comm, MPI_STATUS_IGNORE);
    }
...
// send and receive to/from left/right
    if (idim > 1) { // otherwise in all processes both, left and right are MPI_PROC_NULL
      ...
      MPI_Irecv(&phi[idx(ie+1-b1,ks+b1)],  1,vertical_border, right,4,comm, &req[1]);
      MPI_Isend(&phi[idx(ie+1-2*b1,ks+b1)],1,vertical_border, right,3,comm, &req[2]);
      MPI_Isend(&phi[idx(is+b1,ks+b1)],    1,vertical_border, left, 4,comm, &req[3]);
      MPI_Waitall(4, req, statuses);
      // or alternatively with sendrecv:
      // MPI_Sendrecv(&phi[idx(ie+1-2*b1,ks+b1)],1,vertical_border, right,3,
      ...
      //              &phi[idx(ie+1-b1,ks+b1)],  1,vertical_border, right,4, comm, MPI_STATUS_IGNORE);
    }
    comm_time = comm_time + MPI_Wtime();
