 ...
 *          mpicc -D BUF_CNT_SIZE=10000 progress-test-ssend.c   *
 *                                                              *
 * Contents: C-Source                                           *
 *                                                              *
 ****************************************************************/
...
int main (int argc, char *argv[])
{
  ...
  if (my_rank == 0) /* synchronous send sender */ 
  {
      int token;
      float *snd_buf;
      ...
      printf("I am %i before Ssend call. This call is expected to be local because the receive (as an MPI_Irecv) is already called in process 1\n", my_rank);
      printf("I am %i: With progress, the following MPI_Ssend will come back after a few micro seconds \n", my_rank );
      printf("I am %i: Without progress, the following MPI_Ssend will wait another 15 seconds until process 1 will call the barrier = unspecific call following the Irecv \n", my_rank );
      time_begin = MPI_Wtime();
      MPI_Ssend(snd_buf, BUF_CNT_SIZE, MPI_FLOAT, 1, 17, MPI_COMM_WORLD);
      time_end = MPI_Wtime();
      printf("I am %i after Ssend call, delta = time %9.6lf sec \n", my_rank, time_end-time_begin );
  ...
  else if (my_rank == 1) /* delayed receiver */
  {
      ...
      printf("                              I am %i: 15 seconds ago, process 0 called MPI_Ssend \n", my_rank );

      printf("                              I am %i before barrier call \n", my_rank);
      printf("                              I am %i: With MPI_Ssend progress in process 0, this barrier has to wait for the barrier in proces 0 coming in 40-15 = 25 sec \n", my_rank );
      printf("                              I am %i: Without MPI_Ssend progress in process 0, the Ssend waited for the 15 sec and therefore the following barrier ihas to wait 40 sec \n", my_rank );
      time_begin = MPI_Wtime();
