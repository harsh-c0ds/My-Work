 ...
 *          process                                             *
 *                                                              *
 * progress-test-bsend-3-processes.c                            *
 * uses MPI_Bsend + MPI_Recv and will demonstrate extreme       *
 * delays if progress happens only inside of MPI routines and   *
 * not in an independently runing additional progress thread.   *
 *                                                              *
 * Contents: C-Source                                           *
 *                                                              *
 ****************************************************************/
...
int main (int argc, char *argv[])
{
  ...
  if (my_rank == 0) {
    ...
    printf("============================================================\n");
    printf("\n");
    printf("The two experiments show what can happen if blocking MPI_Bsend and MPI_Recv is used\n");
    printf("to implement a 1-dimensional non-periodic halo communication pattern.\n");
    printf("\n");
    printf("\n");
    printf("If the MPI library implements progress only within MPI procedure calls,\n");
    printf("then each MPI_Recv needs an unspecific MPI call in the sending process,\n");
    printf("because the buffered data resides still there.\n");
    printf("By good luck, this is an MPI_Recv call in the sending process.\n");
    printf("By bad luck,  all such MPI_Recv in the sending process are already done and\n");
    printf("it is already in the numerical work and such unspecific MPI call will be\n");
    printf("not before the numerical epoch ends (and will be the MPI_Bsend in the next\n");
    printf("global iteration).\n");
    printf("\n");
    printf("Therefore, smallest (natural) dalays between the processes can cause that\n");
    printf("each numerical round (i.e., halo exchange plus numerical epoch) needs\n");
    printf("twice the time than expected.\n");
    printf("\n");
    printf("In the first experiment the output should show this as predictable behavior.\n");
    printf("For this, we use dedicated delays between the MPI_Bsend and the MPI_Recv calls.\n");
    printf("\n");
    printf("In the second experiment, these delays are removed and usually the extreme\n");
    printf("slow down is still visible (probably through smallest natural delays).\n");
    printf("\n");
...
// next message may be sent before first message is received on the destination process ==> *2
...
//       into 2 directions (my_rank-1 and my-rank+1) ==> *2, i.e., in total *4
  if ((my_rank == 0) || (my_rank == (size-1))) {
    bsend_buffer_size = ( bsend_buffer_size + MPI_BSEND_OVERHEAD ) * 2;
  }else{
    bsend_buffer_size = ( bsend_buffer_size + MPI_BSEND_OVERHEAD ) * 4;
  }
