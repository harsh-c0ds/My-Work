...
{
  int my_rank, size, thread_provided;
      ...
      printf("\n  provided: %s\n\n",
       (thread_provided == MPI_THREAD_SINGLE ? "MPI_THREAD_SINGLE" :
        (thread_provided == MPI_THREAD_FUNNELED ? "MPI_THREAD_FUNNELED" :
         (thread_provided == MPI_THREAD_SERIALIZED ? "MPI_THREAD_SERIALIZED" : "MPI_THREAD_ unknown"
      ))));
    }
    MPI_Finalize();
    exit(0);
  ...
  if (my_rank == 0) /* buffered sender */ 
  {
      ...
      printf("\nMPI_Init(...,MPI_THREAD_MULTIPLE,...) was called and provided\n");
