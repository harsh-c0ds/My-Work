...
/* ===> 2 <=== */
  ...
  if ( rank_shm == 0 || rank_shm == 1 || rank_shm == size_shm - 1 )
  {
    printf("\t\tprocess %i   arrSize %i arrSize_ %i  shm_buf_ptr = %p, arr_ptr = %p \n",
                rank_world, arrSize, (int) (arrSize_), shm_buf_ptr, arr );
  }
...
/* ===> 7 <=== */
  MPI_Win_fence(/*workaround: no assertions:*/ 0, win); // free destroys the shm. fence to guarantee that read epoch has been finished
  MPI_Win_free(&win);
