...
/* ===> 2 <=== */
  ...
  MPI_Info info;
  // MPI_Info_create (&info);   MPI_Info_set(info, "SHMEM_INFO_KEY", "socket");
  // MPI_Info_create (&info);   MPI_Info_set(info, "SHMEM_INFO_KEY", "package");
  MPI_Info_create (&info);   MPI_Info_set(info, "SHMEM_INFO_KEY", "HWLOC_OBJ_PACKAGE");
  MPI_Comm_split_type(comm_nodes, MPIX_COMM_TYPE_NEIGHBORHOOD, /*key=*/ 0, info, &comm_shm);
