...
/* if (left == -1) left = size-1;*/
  ...
  for( i = 0; i < size; i++) 
  {
    MPI_Issend(&snd_buf, 1, send_recv_type, right, to_right, MPI_COMM_WORLD, &request);  /* Step 2a */
    MPI_Recv(&rcv_buf, 1, send_recv_type, left, to_right, MPI_COMM_WORLD, &status);      /* Step 3 */
