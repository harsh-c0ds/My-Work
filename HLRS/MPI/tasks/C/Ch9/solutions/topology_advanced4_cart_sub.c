...
int main (int argc, char *argv[])
{
  ...
  {
    MPI_Issend(&snd_buf, 1, MPI_INT, right, 17, slice_comm, &request);
    
    MPI_Recv  (&rcv_buf, 1, MPI_INT, left,  17, slice_comm, &status);
  ...
  printf ("PE%2i, Coords=(%i,%i), Slice_rank=%i: Sum = %i\n", 
