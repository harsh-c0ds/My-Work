...
int main (int argc, char *argv[])
{
  ...
  array_of_blocklengths[0] = ___;   // PLEASE SUBSTITUTE ALL SKELETON CODE: ____
  ...
  MPI____________(&send_recv_type);

/* ---------- original source code from MPI/course/C/Ch4/ring.c - PLEASE MODIFY WHERE NEEDED: */
  sum.i = 0;            sum.f = 0;
  snd_buf.i = my_rank;  snd_buf.f = 10*my_rank; // Step 1 = init

  for( i = 0; i < size; i++) 
  {
    MPI_Issend(&snd_buf, 1, MPI_INT, right, 17, MPI_COMM_WORLD, &request);        // Step 2a
    MPI_Recv(&rcv_buf, 1, MPI_INT, left, 17, MPI_COMM_WORLD, &status);            // Step 3
