...
int main (int argc, char *argv[])
{
  ...
  { /* to check, whether the data transfer is correct, we do not transfer the last index */
    MPI_Issend(&snd_buf, arr_lng-1, send_recv_type, right, 17, MPI_COMM_WORLD, &request);  /* Step 2a */
    MPI_Recv(&rcv_buf, arr_lng-1, send_recv_type, left, 17, MPI_COMM_WORLD, &status);      /* Step 3 */
  ...
  if (my_rank==0)
  {
    ...
    printf("E-- real size is:             %3d\n", (int)(MPI_Aint_diff(second_var_address,first_var_address)));
    if (buf_mpi_extent != (MPI_Aint_diff(second_var_address,first_var_address)))
    {
      printf("F--  CAUTION:  mismatch of language type and MPI derived type: %3d != %3d\n",
             (int)buf_mpi_extent, (int)(MPI_Aint_diff(second_var_address,first_var_address)));
    }
    printf("G--\n");
    printf("H-- Expected results: for all, except the highjest j:  sum = (j+1)*(sum of all ranks)\n");
    printf("I-- For the highest j value, no data exchange is done: sum = -(number of processes)\n");
  }
