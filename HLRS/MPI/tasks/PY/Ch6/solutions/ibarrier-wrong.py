...
snd_buf_A = np.empty(1, dtype=np.intc); snd_buf_B = np.empty(1, dtype=np.intc)
snd_buf_C = np.empty(1, dtype=np.intc); snd_buf_D = np.empty(1, dtype=np.intc)
...
rcv_buf = np.empty(1, dtype=np.intc)
ib_finished=False
rcv_sts = MPI.Status()
...
while( not ib_finished):
   ...
   if(rcv_flag):
      ...
      print("A rank: {0:3d} - received: message {1:06d} from {2:3d} to {0:3d}".format(my_rank,rcv_buf[0],rcv_sts.Get_source()))
