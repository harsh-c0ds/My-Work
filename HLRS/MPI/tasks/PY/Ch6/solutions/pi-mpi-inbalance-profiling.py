...
comm_world.Barrier() # for a nearly common start time
...
comm_world.Barrier() # to profile the idle time due to bad load balance
wt3=MPI.Wtime() # before communication

comm_world.Reduce(p_sum, (sum, 1, MPI.DOUBLE), op=MPI.SUM, root=0)

comm_world.Barrier() # for a nearly common end time
...
wt_total     = wt4 - wt1; wt_all_total   =   comm_world.reduce(wt_total, op=MPI.SUM, root=0) # root=0 and op=MPI.SUM are default, we omit them in the following
wt_numerics  = wt2 - wt1; wt_all_numerics  = comm_world.reduce(wt_numerics)
wt_inbalance = wt3 - wt2; wt_all_inbalance = comm_world.reduce(wt_inbalance)
wt_comm      = wt4 - wt3; wt_all_comm   =    comm_world.reduce(wt_comm)
...
comm_world.Barrier() # to have some chance to separate output above and below in common stdout
if (my_rank == 0):
   pi=w*sum
