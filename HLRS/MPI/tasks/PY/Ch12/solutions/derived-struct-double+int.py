...
# Use aligned datatype, so contiguity is not guaranteed.
# What happens if align=False? Why?
...
for i in range(size):
   ...
   request = comm_world.Issend((snd_buf, arr_lng-1, send_recv_type), right, 17)  # Step 2a
   comm_world.Recv((rcv_buf, arr_lng-1, send_recv_type), left, 17, status);      # Step 3
...
if (my_rank==0):
   buf_mpi_size = send_recv_type.Get_size()
   (buf_mpi_lb, buf_mpi_extent) = send_recv_type.Get_extent()
   (buf_mpi_lb, buf_mpi_true_extent) = send_recv_type.Get_true_extent()
   ...
   print("D-- sizeof:                   {:3d}".format(sum.itemsize))
   mem = MPI.memory.frombuffer(sum)
   print("E-- real size is:             {:3d}".format( int(mem.nbytes/sum.size) ))
   if (buf_mpi_extent != int(mem.nbytes/sum.size)):
      print("F--  CAUTION:  mismatch of language type and MPI derived type: {:3d} != {:3d}".format(
             buf_mpi_extent, int(mem.nbytes/sum.size)))

   print("G--")
   print("H-- Expected results: for all, except the highjest j:  sum = (j+1)*(sum of all ranks)")
   print("I-- For the highest j value, no data exchange is done: sum = -(number of processes)")
