...
if ( rank_shm == 0 or rank_shm == 1 or rank_shm == size_shm - 1 ):  # TEST #
   print("\t\trank ( world: {}, shm: {}, head: {}) arrSize {} arrSize_ {}  shm_buf_ptr = 0x{:x}, arr_ptr = 0x{:x} ".format(
                    rank_world, rank_shm, rank_head, arrSize, arrSize_, shm_buf_ptr, buf.address )) 
if(rank_world==0):                                                  # TEST #
   print(f"ALL finalize and return !!!."); 

sys.exit(0) # Remeber sys.exit() still calls all exit handlers, i.e. also MPI.Finalize()
