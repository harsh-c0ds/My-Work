...
np_dtype = np.intc
size_of_type = np_dtype(0).itemsize
...
# Create the window.
mem = MPI.Alloc_mem(1*size_of_type, MPI.INFO_NULL)
win = MPI.Win.Create(memory=mem, disp_unit=size_of_type, info=MPI.INFO_NULL, comm=comm_world)

# To work with this bunch of memory we consider it as numpy array
rcv_buf = np.frombuffer(mem, dtype=np_dtype)
