...
if (my_rank == 0): # ready-send send sender
   ...
   print("I am {}: With progress, the following MPI_Rsend will come back after a few micro seconds ".format(my_rank ))
   print("I am {}: Without progress, the following MPI_Rsend will wait another 15 seconds until process 1 will call the barrier = unspecific call following the Irecv ".format(my_rank ))
   time_begin = MPI.Wtime();
   comm_world.Rsend((snd_buf, BUF_CNT_SIZE, MPI.FLOAT), 1, 17)
   time_end = MPI.Wtime()
   print("I am {} after Rsend call, delta = time {:9.6f} sec ".format(my_rank, time_end-time_begin ))
   ...
   print("                              I am {}: 15 seconds ago, process 0 called MPI_Rsend ".format(my_rank ))

   print("                              I am {} before barrier call ".format(my_rank))
   print("                              I am {}: With MPI_Rsend progress in process 0, this barrier has to wait for the barrier in proces 0 coming in 40-15 = 25 sec ".format(my_rank ))
   print("                              I am {}: Without MPI_Rsend progress in process 0, the Send waited for the 15 sec and therefore the following barrier ihas to wait 40 sec ".format(my_rank ))
   time_begin = MPI.Wtime()
