...
# send and receive to/from upper/lower
    if (kdim > 1) : # otherwise in all processes both, lower and upper are MPI_PROC_NULL
        req[0] = comm.Irecv((phi[idx(iss+b1,ks       ):], 1,horizontal_border), source=lower,tag=1)
        req[1] = comm.Irecv((phi[idx(iss+b1,ke+1-b1  ):], 1,horizontal_border), source=upper,tag=2)
        req[2] = comm.Isend((phi[idx(iss+b1,ke+1-2*b1):], 1,horizontal_border), dest=upper,tag=1)
        req[3] = comm.Isend((phi[idx(iss+b1,ks+b1    ):], 1,horizontal_border), dest=lower,tag=2)
        MPI.Request.Waitall(req, statuses)
        # or alternatively with sendrecv:
        # comm.Sendrecv((phi[idx(iss+b1,ke+1-2*b1):], 1,horizontal_border)), dest=upper,  sendtag=1,
        #               (phi[idx(iss+b1,ks       ):], 1,horizontal_border)), source=lower,recvtag=1, MPI.STATUS_IGNORE)
        # comm.Sendrecv((phi[idx(iss+b1,ks+b1    ):],1,horizontal_border)), dest=lower,  sendtag=2,
        #               (phi[idx(iss+b1,ke+1-b1  ):],   1,horizontal_border)), source=upper,recvtag=2, MPI.STATUS_IGNORE)


# send and receive to/from left/right
    if (idim > 1) : # otherwise in all processes both, left and right are MPI_PROC_NULL
        req[0] = comm.Irecv((phi[idx(iss,      ks+b1):],1,vertical_border), source=left, tag=3)
        req[1] = comm.Irecv((phi[idx(ie+1-b1,  ks+b1):],1,vertical_border), source=right,tag=4)
        req[2] = comm.Isend((phi[idx(ie+1-2*b1,ks+b1):],1,vertical_border), dest=right,tag=3)
        req[3] = comm.Isend((phi[idx(iss+b1,   ks+b1):],1,vertical_border), dest=left, tag=4)
        MPI.Request.Waitall(req, statuses);
        # or alternatively with sendrecv:
        # comm.Sendrecv(phi[idx(ie+1-2*b1,ks+b1):],1,vertical_border, right,3,
        #               phi[idx(iss,      ks+b1):],1,vertical_border, left, 3, MPI.STATUS_IGNORE)
        # comm.Sendrecv(phi[idx(iss+b1,   ks+b1):],1,vertical_border, left, 4,
        #               phi[idx(ie+1-b1,  ks+b1):],1,vertical_border, right,4, MPI.STATUS_IGNORE)
