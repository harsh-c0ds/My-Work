...
C                                                              C  
C Purpose: A program to try MPI_Irecv and MPI_Issend.          C
C                                                              C
...
C     IF (left .EQ. -1) left = size-1
      ...
      DO i = 1, size
         ...
         CALL MPI_IRECV(rcv_buf, 1, MPI_INTEGER, left, to_right,
     &                  MPI_COMM_WORLD, arr_request(1), ierror)

         CALL MPI_ISSEND(snd_buf, 1, MPI_INTEGER, right, to_right,
     &                  MPI_COMM_WORLD, arr_request(2), ierror)
