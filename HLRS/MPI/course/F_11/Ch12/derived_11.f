...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      INTEGER int_snd_buf, int_rcv_buf, int_sum
      REAL real_snd_buf, real_rcv_buf, real_sum

      COMMON /send_block/ int_snd_buf, real_snd_buf
      COMMON /recv_block/ int_rcv_buf, real_rcv_buf
...
C Create derived datatype. 
      ...
      CALL MPI_ADDRESS(int_snd_buf, first_var_address, ierror)
      CALL MPI_ADDRESS(real_snd_buf, second_var_address, ierror)
...
C Initialise data
      ...
      int_snd_buf = my_rank
      real_snd_buf = REAL(my_rank)

C Compute sum
      DO i = 1, size

         CALL MPI_ISSEND(int_snd_buf, 1, send_recv_type, right, 
     &                   to_right, MPI_COMM_WORLD, request, ierror)

         CALL MPI_RECV(int_rcv_buf, 1, send_recv_type, left, 
     &                 to_right, MPI_COMM_WORLD, status, 
     &                 ierror)

         CALL MPI_WAIT(request, status, ierror)

         int_sum = int_sum + int_rcv_buf
         real_sum = real_sum + real_rcv_buf

         int_snd_buf = int_rcv_buf
         real_snd_buf = real_rcv_buf
