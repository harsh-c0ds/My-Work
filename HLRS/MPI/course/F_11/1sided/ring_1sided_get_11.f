...
C          with window=snd_buf and MPI_GET to get              C
C          remote window (snd_buf) value into local rcv_buf.   C
C                                                              C
...
C CREATE THE WINDOW.
      ...
      CALL MPI_WIN_CREATE(snd_buf, rcv_buf_size, disp_unit,
...
C         ... The compiler may move the write access to snd_buf
C             in the previous loop iteration (e.g. by internally 
C             using a register instead of memory) after the following 
C             1-sided MPI calls, because the compiler has no chance
C             to see, that snd_buf will be modified by the following 
...
C         ... Now, the compiler expects that snd_buf was modified,
C             because the compiler cannot see that MPI_GET_ADDRESS
C             did nothing. Therefore the compiler cannot move any
C             access to snd_buf across this dummy call. 
 
          CALL MPI_WIN_FENCE(MPI_MODE_NOSTORE + MPI_MODE_NOPRECEDE,
     &                       win, ierror)
          CALL MPI_GET(rcv_buf, 1, MPI_INTEGER, left,
     &                 target_disp, 1, MPI_INTEGER, win, ierror)
          CALL MPI_WIN_FENCE(MPI_MODE_NOSTORE + MPI_MODE_NOPUT +
     &                       MPI_MODE_NOSUCCEED, win, ierror)
 
C         ... The compiler has no chance to see, that snd_buf was
C             read form another process between the two FENCEs. 
C             I compiler would be allowed to move the following 
C             assignment before the second FENCE. To prohibit
C             such a code move, a dummy routine must be called
C             with snd_buf as argument:
...
C         ... This dummy call with rcv_buf in the argument list prevents
C             the following load access to rcv_buf may be moved by the
C             compiler to a position between PUT(rcv_buf...) and FENCE.
