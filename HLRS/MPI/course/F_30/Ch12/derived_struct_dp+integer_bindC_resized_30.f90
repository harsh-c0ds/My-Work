...
!    ... to check, whether the data transfer is correct, we do not transfer the last index
     CALL MPI_Issend(snd_buf, arr_lng-1, send_recv_resized, right, to_right, MPI_COMM_WORLD, request)

     CALL MPI_Recv(rcv_buf, arr_lng-1, send_recv_resized, left, to_right, MPI_COMM_WORLD, status)
