...
!**************************************************************!
  ...
  INTEGER :: my_world_rank, world_size, my_new_rank, new_size
  ...
  TYPE(MPI_Comm ) :: new_comm, comm_cart
  ...
  ! Establishing new_comm with the new ranking in a array "ranks":
    ...
    CALL MPI_Group_incl(world_group, world_size, ranks, new_group)
    CALL MPI_Comm_create(MPI_COMM_WORLD, new_group, new_comm)

  ! testing the new communicator, e.g., with our ring algorithm:
    CALL MPI_Comm_size(new_comm, new_size)  ! should be the original one
    CALL MPI_Comm_rank(new_comm, my_new_rank)
    ...
    CALL MPI_Cart_create(new_comm, 2, dims, periods, .FALSE., comm_cart) ! No reorder !!!
    CALL MPI_Cart_coords(comm_cart, my_new_rank, 2, coords)
...
! ----------------------------------------------------------------------
    ...
    CALL MPI_Cart_shift(comm_cart, 0, 1, left, right)  ! dir=0 !!!
    ! coparison of the results
    ...
    DO i=1, dim0
      ...
      CALL MPI_Issend(snd_buf, 1, MPI_INT, right, to_right, comm_cart, request)
      CALL MPI_Recv(rcv_buf, 1, MPI_INT, left, to_right, comm_cart, status)
...
! ----------------------------------------------------------------------
    ...
    ! coordinates through a cartesian Virtual toplogy based on new_comm
    ! right=..., left=... should be substituted by one call to MPI_Cart_shift():
    CALL MPI_Cart_shift(comm_cart, 1, 1, left, right)  ! dir=1 !!!
    ! coparison of the results
