...
!         *** PROFILING: communication epoch, BEGIN
          ...
          IF (my_rank .EQ. 0) THEN
            ...
            CALL MPI_Recv(rcv1buf, BUFLNG, MPI_INTEGER, left,  111, MPI_COMM_WORLD, status, ierror)
          ELSE
            CALL MPI_Recv(rcv1buf, BUFLNG, MPI_INTEGER, left,  111, MPI_COMM_WORLD, status, ierror)
            CALL MPI_Send (snd1buf, BUFLNG, MPI_INTEGER, right, 111, MPI_COMM_WORLD, ierror)
          END IF
          ...
          IF (my_rank .EQ. 0) THEN
            ...
            CALL MPI_Recv(rcv2buf, BUFLNG, MPI_INTEGER, right, 222, MPI_COMM_WORLD, status, ierror)
          ELSE
            CALL MPI_Recv(rcv2buf, BUFLNG, MPI_INTEGER, right, 222, MPI_COMM_WORLD, status, ierror)
            CALL MPI_Send (snd2buf, BUFLNG, MPI_INTEGER, left,  222, MPI_COMM_WORLD, ierror)
          END IF
...
!       *** PROFILING: numerics, are not parallelized, END
        ...
        lng_unbalance = (1.0 * my_rank / size) * LOOPUNBALANCE
        IF (mod(my_rank,2) .EQ. 0) THEN
          lng_unbalance = 0
        END IF
!       *** minimal loop length = 0
!       *** maximal loop length = LOOPUNBALANCE * (size-1) / size   (if size is even)
!       *** average loop length = LOOPUNBALANCE * size / 4          (if size is even)

        DO i=1,lng_unbalance
          x = x / 2
          s = s + x
        END DO
