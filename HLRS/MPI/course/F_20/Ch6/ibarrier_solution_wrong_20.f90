...
! ...in the role as sending process
  ...
  IF ((dest>=0) .AND. (dest<size)) THEN
    ...
    CALL MPI_ISEND(snd_buf_A,1,MPI_INTEGER, dest,222,MPI_COMM_WORLD, snd_rq(number_of_dests), ierror) ! WRONG, must by sync. Issend
  ...
  IF ((dest>=0) .AND. (dest<size)) THEN
    ...
    CALL MPI_ISEND(snd_buf_B,1,MPI_INTEGER, dest,222,MPI_COMM_WORLD, snd_rq(number_of_dests), ierror) ! WRONG, must by sync. Issend
  ...
  IF ((dest>=0) .AND. (dest<size)) THEN
    ...
    CALL MPI_ISEND(snd_buf_C,1,MPI_INTEGER, dest,222,MPI_COMM_WORLD, snd_rq(number_of_dests), ierror) ! WRONG, must by sync. Issend
  ...
  IF ((dest>=0) .AND. (dest<size)) THEN
    ...
    CALL MPI_ISEND(snd_buf_D,1,MPI_INTEGER, dest,222,MPI_COMM_WORLD, snd_rq(number_of_dests), ierror) ! WRONG, must by sync. Issend
