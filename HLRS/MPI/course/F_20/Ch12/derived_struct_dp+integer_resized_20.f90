...
! Create derived datatype. 
  ...
  CALL MPI_GET_ADDRESS(snd_buf(1), first_var_address, ierror)
  CALL MPI_GET_ADDRESS(snd_buf(2), second_var_address, ierror)
  lb = 0
  extent = second_var_address - first_var_address
  CALL MPI_TYPE_CREATE_RESIZED(send_recv_type, lb, extent, send_recv_resized, ierror)
  CALL MPI_TYPE_COMMIT(send_recv_resized, ierror)
...
!    ... to check, whether the data transfer is correct, we do not transfer the last index
     CALL MPI_ISSEND(snd_buf, arr_lng-1, send_recv_resized, right, to_right, MPI_COMM_WORLD, request, ierror)

     CALL MPI_RECV(rcv_buf, arr_lng-1, send_recv_resized, left, to_right, MPI_COMM_WORLD, status, ierror)
...
!    IF (.NOT.MPI_ASYNC_PROTECTS_NONBLOCKING) CALL MPI_F_sync_reg(snd_buf)
  ...
  if (my_rank==0) THEN
    ...
    CALL MPI_Type_size(send_recv_resized, buf_mpi_size, ierror)
    CALL MPI_Type_get_extent(send_recv_resized, buf_mpi_lb, buf_mpi_extent, ierror)
    CALL MPI_Type_get_true_extent(send_recv_resized, buf_mpi_lb, buf_mpi_true_extent, ierror)
    ...
    WRITE(*,"(a,i3)") "H-- real size is:             ", INT(second_var_address-first_var_address)
