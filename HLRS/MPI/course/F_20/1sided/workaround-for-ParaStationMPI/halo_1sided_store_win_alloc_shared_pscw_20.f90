...
! ...ParaStation MPI may not allow MPI_Win_allocate_shared on MPI_COMM_WORLD. Workaround: Substitute MPI_COMM_WORLD by comm_sm (on 2 lines):
  CALL MPI_WIN_ALLOCATE_SHARED(buf_size, disp_unit, MPI_INFO_NULL, comm_sm, ptr_rcv_buf_left, win_rcv_buf_left, ierror)
...
!                                 rcv_buf_left(xxx) in process 'left':
  ...
  CALL MPI_WIN_ALLOCATE_SHARED(buf_size, disp_unit, MPI_INFO_NULL, comm_sm, ptr_rcv_buf_right, win_rcv_buf_right, ierror)
...
!       ...workaround: no assertions for shared memory - MPI-3.0 and MPI-3.1 are not clear:
        CALL MPI_WIN_POST (grp_left,  0, win_rcv_buf_left,  ierror)
        CALL MPI_WIN_POST (grp_right, 0, win_rcv_buf_right, ierror)
