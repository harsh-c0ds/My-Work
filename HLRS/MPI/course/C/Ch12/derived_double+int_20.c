...
/* if (left == -1) left = size-1;*/
  ...
  {
    sum[j].i = 0;            sum[j].f = 0;
    snd_buf[j].i = my_rank;  snd_buf[j].f = my_rank;  /* Step 1 = init */
    rcv_buf[j].i = -1;  rcv_buf[j].f = -1;
  }

  for( i = 0; i < size; i++) 
  { /* to check, whether the data transfer is correct, we do not transfer the last index */
    MPI_Issend(&snd_buf, arr_lng-1, send_recv_type, right, to_right, MPI_COMM_WORLD, &request);  /* Step 2a */
    MPI_Recv(&rcv_buf, arr_lng-1, send_recv_type, left, to_right, MPI_COMM_WORLD, &status);      /* Step 3 */
