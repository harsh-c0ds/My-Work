...
{ double s, x;
  ...
  if ((my_rank % 2) == 0) lng_unbalance = 0;
...
#   endif
    ...
    if (my_rank==0) {
      ...
      MPI_Recv(rcv1buf, BUFLNG, MPI_INT, left,  111, MPI_COMM_WORLD, &status);
    }else{
      MPI_Recv(rcv1buf, BUFLNG, MPI_INT, left,  111, MPI_COMM_WORLD, &status);
      MPI_Send (snd1buf, BUFLNG, MPI_INT, right, 111, MPI_COMM_WORLD);
    }
    ...
    if (my_rank==0) {
      ...
      MPI_Recv(rcv2buf, BUFLNG, MPI_INT, right, 222, MPI_COMM_WORLD, &status);
    }else{
      MPI_Recv(rcv2buf, BUFLNG, MPI_INT, right, 222, MPI_COMM_WORLD, &status);
      MPI_Send (snd2buf, BUFLNG, MPI_INT, left,  222, MPI_COMM_WORLD);
    }
