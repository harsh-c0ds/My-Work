...
/* if (left == -1) left = size-1;*/
  ...
  MPI_Win_allocate_shared((MPI_Aint)(1*sizeof(int)), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD,&signal_A_ptr, &win_signal_A);
  *signal_A_ptr = 0;
  MPI_Win_fence(0,win_signal_A);
  MPI_Win_lock_all(MPI_MODE_NOCHECK, win_signal_A);

  MPI_Win_allocate_shared((MPI_Aint)(1*sizeof(int)), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD,&signal_B_ptr, &win_signal_B);
  *signal_B_ptr = 0;
