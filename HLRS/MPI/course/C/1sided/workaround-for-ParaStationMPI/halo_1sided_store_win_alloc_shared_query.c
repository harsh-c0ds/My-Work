...
/*              halo-communication                 halo-communication                */
  ...
  MPI_Win_allocate_shared((MPI_Aint)(max_length*sizeof(float)), sizeof(float), info_noncontig, comm_sm, &rcv_buf_left,  &win_rcv_buf_left );
  MPI_Win_allocate_shared((MPI_Aint)(max_length*sizeof(float)), sizeof(float), info_noncontig, comm_sm, &rcv_buf_right, &win_rcv_buf_right);
...
/*... shared memory access to the rcv_buf_right, of the LEFT neighbor process */
  ...
  { 
    ...
    for (i = 0; i <= number_of_messages; i++)
    {
      ...
      MPI_Win_fence(/*workaround: no assertions:*/ 0, win_rcv_buf_left );
      MPI_Win_fence(/*workaround: no assertions:*/ 0, win_rcv_buf_right);
